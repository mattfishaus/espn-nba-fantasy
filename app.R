...
server <- function(input, output) {
  ...
  output$resultStandings <- DT::renderDataTable(
    ...
    rownames = FALSE, 
    options = list(autoWidth = FALSE, columnDefs = list(list(className='dt-left',sClass="alignLeft", width = '100px', targets = "_all")), pageLength = 12)
    ))
  ...
  class = 'table-bordered stripe table-condensed',  
  rownames = FALSE, 
  options = list(autoWidth = FALSE, columnDefs = list(list(className='dt-left', sClass="alignLeft", width = '100px', targets = "_all")), pageLength = 100)
  ))
  }
